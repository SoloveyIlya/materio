...
const WelcomeCard = () => {
  ...
  const data = [
    {
      title: 'Start Date',
      value: dashboardData?.work_start_date 
        ? new Date(dashboardData.work_start_date).toLocaleDateString('en-US', { day: '2-digit', month: '2-digit', year: 'numeric' })
        : 'Not set',
      color: 'primary',
      icon: (
    ...
    {
      title: 'Work Days',
      value: dashboardData?.work_days || '0',
      color: 'info',
      icon: (
        <svg xmlns='http://www.w3.org/2000/svg' width='38' height='38' viewBox='0 0 38 38' fill='none'>
          <path
    ...
    {
      title: 'Timezone',
      value: dashboardData?.timezone || 'UTC',
      color: 'warning',
      icon: (
        <svg xmlns='http://www.w3.org/2000/svg' width='38' height='38' viewBox='0 0 38 38' fill='none'>
          <path
    ...
    <div className='flex max-md:flex-col md:items-center gap-6 plb-5'>
      <div className='md:is-8/12'>
        <div className='flex items-baseline gap-1 mbe-2'>
          ...
          <Typography variant='h4'>{user.name || user.email} 👋🏻</Typography>
        </div>
        <div className='mbe-4'>
          <Typography>New tasks and reviews are waiting for you. Let&apos;s get started.</Typography>
        </div>
        <div className='flex flex-wrap max-md:flex-col justify-between gap-6'>
          {data.map((item, i) => (
            <div key={i} className='flex gap-4'>
      ...
      <div className='flex justify-between md:is-4/12'>
        <div className='flex flex-col justify-between gap-6'>
          ...
          <div>
            ...
            <Chip label={`${dashboardData.success_rate || 0}% Success`} variant='tonal' size='small' color='success' />
          </div>
        </div>
      </div>
    </div>
